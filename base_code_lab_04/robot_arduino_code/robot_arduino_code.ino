...
void lidar_update() {
  if (IS_OK(lidar.waitPoint())) {
    ...
    static unsigned long last_lidar_error_time = 0;
    if (millis() - last_lidar_error_time > 1000) {
      Serial.println("[DEBUG] Failed to read LIDAR point. Is the motor spinning?");
      last_lidar_error_time = millis();
    }
    // -----------------------------
...
void send_sensor_signal(SensorSignal sensor_signal)
{
  ...
  if (new_time_tx - last_time_tx > SendDeltaTimeInMs) {
    ...
    // --- ADDED DEBUGGING LOGIC ---
    Serial.print("[DEBUG] Sending packet... LIDAR Rays Packaged: ");
