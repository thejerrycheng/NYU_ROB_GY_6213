...
// char remoteIP[] = "192.168.1.182"; //"192.168.50.196"; // REPLACE with your laptop's IP address on your team's router
// unsigned int localPort = 4010;     // local port to listen on - no need to change
// unsigned int remotePort = 4010;    // local port to listen on - no need to change
// int status = WL_IDLE_STATUS;
// int last_time_rx = 0;
...
// // Bind the RPLIDAR driver to hardware serial 1 (Pins 19 and 18)
//   Serial1.begin(115200); // Standard A1/A2 baud rate. Change to 256000 for newer models like A3.
...
//   // Activate the Lidar motor
//   pinMode(RPLidarMotorPin, OUTPUT);
//   analogWrite(RPLidarMotorPin, 255); // 255 is max speed; tune this down if it spins too fast
...
//   if (new_time_tx - last_time_tx > SendDeltaTimeInMs) {
//     // For debugging: 
//     Serial.print("Lidar rays packaged: ");
//     Serial.println(current_num_lidar_rays);
