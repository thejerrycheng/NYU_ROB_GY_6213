...
# HYPERPARAMETERS
# ==========================================
# Gmapping / Particle Filter Params
NUM_PARTICLES = 20  # Increased for better swarm diversity
...
ROBOT_RADIUS           = 0.15
# INCREASED CLEARANCE: Prevents A* from cutting corners and causing physical collisions
PLANNER_WALL_CLEARANCE = 0.35  
...
class FastSLAM:
    ...
    def update(self, angles, distances, max_range=5.0):
        slam_angles = angles[::3]
        slam_distances = distances[::3]
        
        weight_sum = 0.0
        
        # HILL-CLIMBING SCAN MATCHER: Test slight variations of the pose to find the sharpest wall alignment
        search_offsets = [
            (0.0, 0.0, 0.0),
            (0.05, 0.0, 0.0), (-0.05, 0.0, 0.0),   # Forward / Backward
            (0.0, 0.05, 0.0), (0.0, -0.05, 0.0),   # Left / Right
            (0.0, 0.0, 0.05), (0.0, 0.0, -0.05)    # Rotate Left / Rotate Right
        ]
        
        # --- A. SCAN MATCHING (WEIGHTING) ---
        for p in self.particles:
            best_score = -1.0
            best_pose = p.pose
            
            for dx, dy, dtheta in search_offsets:
                # Transform local shifts to global frame
                test_pose = np.array([
                ...
                for i in range(len(slam_angles)):
                    ...
                    if 0 <= gx < self.W and 0 <= gy < self.H:
                        val = p.grid[gx, gy]
                        if val > 0: # Directly reward alignment with existing log-odds walls
                            score += val 
            ...
            # Commit the particle to its newly optimized pose
            p.pose = best_pose
        ...
        if n_eff < PF_RESAMPLE_THRESHOLD:
            new_particles = []
...
class ActiveSLAMController:
    ...
    def pd_controller(self, robot_pose, dt=0.1):
        ...
        if self.stuck_check_pose is None: self.stuck_check_pose = [rx, ry]
            ...
            if math.hypot(rx - self.stuck_check_pose[0], ry - self.stuck_check_pose[1]) < self.STUCK_DIST_MIN:
                ...
                self.recovery_steps = 8; return -self.MIN_V_CMD, 0.0
        ...
        wp_x, wp_y = self.current_path[min(1, len(self.current_path) - 1)]
        ...
        alpha_cmd = float(np.clip(-(self.KP_steer * heading_err + self.KD_steer * d_heading), -MAX_ALPHA_CMD, MAX_ALPHA_CMD))
        ...
        v_cmd = max(self.MIN_V_CMD, base_v_cmd * 0.5) if abs(heading_err) > 0.35 else base_v_cmd
    ...
    def update(self, robot_pose):
        ...
        if self.current_path: return self.pd_controller(robot_pose)
...
def run_sim(map_name):
    ...
    while plt.fignum_exists(fig1.number) and plt.fignum_exists(fig2.number):
        ...
        v_cmd, alpha_cmd = ai_controller.update(best_pose)
