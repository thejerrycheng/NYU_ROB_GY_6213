...
def run_sim(map_name, episode_idx):
    ...
    step = 0
    print(f"Worker started: Episode {episode_idx}...")
    ...
    while step < MAX_STEPS:
        ...
        # Log Data
        episode_data['v_cmds'].append(v_cmd)
        episode_data['alpha_cmds'].append(alpha_cmd)
        episode_data['lidar_angles'].append(angles)
        episode_data['lidar_distances'].append(distances)
        episode_data['true_poses'].append(true_pose.copy())
    ...
    if step >= MAX_STEPS:
         print(f"Episode {episode_idx} hit max steps ({MAX_STEPS}). Terminating early.")

    # 1. Save the Final Map Grid as a PNG
    final_map_probs = slam.get_best_probabilities()
    
    # Scale probabilities to 0-255 grayscale (0.0=Free=255, 1.0=Occupied=0)
    pixel_data = ((1.0 - final_map_probs) * 255).astype(np.uint8)
    
    # Transpose and flip up/down so it visually matches the simulated coordinates
    pixel_data = np.flipud(pixel_data.T)
    
    img = Image.fromarray(pixel_data, mode='L')
    map_save_path = os.path.join(f"saved_{map_name}", f"map_ep_{episode_idx}.png")
    img.save(map_save_path)

    # 2. Save the Trajectory / Sensor Data
    data_save_path = os.path.join("data", f"{map_name}_ep_{episode_idx}_data.pkl")
...
if __name__ == '__main__':
    ...
    os.makedirs(f"saved_{args.map}", exist_ok=True)
    os.makedirs("data", exist_ok=True)
    
    print(f"Starting Parallel Headless Data Collection on Map: {args.map}")
    print(f"Target: {args.episodes} episode(s).")
    print("=======================================================")
    ...
    print("\n=======================================================")
    print("All episodes successfully processed and saved!")